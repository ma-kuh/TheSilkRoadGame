...
def setup_game(num_players=3):
    ...
    top_space = GAME_SIZE[1] - (VER_PLAYMAT_SIZE[1]  *  (num_players % 3)) - BOARD_IMG_SIZE[1]
    board_space = Space(
        (HOR_PLAYMAT_SIZE[0] + (GAME_SIZE[0] - HOR_PLAYMAT_SIZE[0]*2 - BOARD_IMG_SIZE[0]) // 2),
        top_space,
        BOARD_IMG_SIZE[0],
        BOARD_IMG_SIZE[1],
    )
    canvas.append(draw.Image(
        board_space.x0,
        board_space.y0,
        board_space.width,
        board_space.height,
...
def draw_bank(canvas, space):
    ...
    for value in (1,3,5,10):
        for idx in range(10):
            canvas.append(draw.Image(
                space.x0 + ((random.random()*BANK_SIZE[0]//3) + (BANK_SIZE[0] // 2)),
                space.y0 + (5 * random.random()*BANK_SIZE[1] //  6),
                COIN_SIZE[0],
                COIN_SIZE[1],
